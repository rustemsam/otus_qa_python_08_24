...
def test_triangle_perimeter(triangle, a_side, b_side, c_side, expected_perimeter):
    ...
    assert (
        round(tri.perimeter, 2) == expected_perimeter
    ), f"Perimeter should be {expected_perimeter}, but got {tri.perimeter}"
...
def test_triangle_area(triangle, a_side, b_side, c_side, expected_area):
    tri = triangle(a_side, b_side, c_side)
    assert (
        round(tri.area, 2) == expected_area
    ), f"Area should be {expected_area}, but got {tri.area}"


@pytest.mark.negative
@pytest.mark.parametrize("a_side, b_side, c_side", [(5, 5, 0), (-1, 5, 5), (4, 0, 5)])
...
@pytest.mark.negative
@pytest.mark.parametrize("a_side, b_side, c_side", [(5, 5, 10), (6, 13, 6), (14, 7, 7)])
